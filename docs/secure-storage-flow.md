...
| Android: RNSI 5.6.2 → 6 native re-shape failed | Storage error screen ("close the app completely and open it again") | `migrate_native_store`: 5.6.2 entry present, marker not `done`; schema stays 0; `MainApplication.onCreate` retries on the next launch | Old blob |
