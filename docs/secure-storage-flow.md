...
- **react-native-sensitive-info 5.6.2 Android store.** 6.x uses a different SharedPreferences layout. `android/.../SensitiveInfoV6Migration.kt` re-shapes every unprotected 5.6.2 entry into the 6.x envelope at `Application.onCreate`, before any JS runs, without decrypting anything (same ciphertext, IV and Keystore key). The 5.6.2 file is retained for now; a marker file `rnsi_v6_migration` makes the step idempotent and records the outcome: `state`=`done` plus one `entry.<service>::<key>` status per 5.6.2 entry (`migrated`, `exists`, or `skipped` for an unparseable/protected one), committed only after the re-shaped data, or `error` after a failed run. A failure never stops React from loading; instead `readLegacyRoot` sees the 5.6.2 `persist:root2` entry still in `sensitive_info` and requires that entry's own status: none → `migrate_native_store` (the next launch re-runs the native step), `skipped` → `migrate_parse`. Either way the blob is never finalised over. iOS needs nothing: both versions store the same Keychain item.
...
| Android: RNSI 5.6.2 → 6 native re-shape failed | Storage error screen ("close the app completely and open it again") | `migrate_native_store`: 5.6.2 entry present without its `entry.…` status; schema stays 0; `MainApplication.onCreate` retries on the next launch | Old blob |
| Android: native re-shape skipped the redux root entry (corrupt/protected 5.6.2 envelope) | Storage error screen (same copy as unreadable data) | `migrate_parse`: entry status `skipped`; schema stays 0 | Old blob (unreadable by 5.6.2 as well) |
