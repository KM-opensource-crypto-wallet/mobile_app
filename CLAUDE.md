...
- **Sensitive Storage**: Uses `react-native-sensitive-info` 6.1.5 (Nitro Modules, New Architecture) for the vault items, the MMKV key and the App Attest state
- **Secure storage / vault (Phases 1–5 landed; QA + R9b measurement pending)**: spec in `docs/superpowers/specs/2026-09-19-secure-storage-final-plan.md` (identical copy in the web repo). Shared, platform-free code lives in the submodule and reaches platform code only through two bare aliases each app resolves to its own file, exactly like `myWallet/wallet.service`: `security/vaultCrypto` (mobile `src/security/vaultCrypto.js` on react-native-quick-crypto; web on WebCrypto + a KDF worker) and `security/secureStore` (mobile on react-native-sensitive-info 6.1.5 under `SECURE_STORE_KEYCHAIN_NAME`; web on IndexedDB). Submodule: `security/vaultCore.js` (envelope v1, PBKDF2-SHA256 600k → AES-256-GCM, `ct` carries the tag, AAD per purpose), `security/vault.js` (DEK in memory, `createVault`/`unlockWithPassword`/`verifyPassword`/`changePassword`/biometric/`saveSecrets`/`getStateKey`/`destroy`), `redux/wallets/walletSecrets.js` (`SECRET_WALLET_FIELDS`, strip/extract/hydrate/`assertNoSecrets`), `walletsSlice.hydrateWalletSecrets`, `auth.hasAccount` + `getHasAccount` (the `password` field and `getUserPassword` are gone: `signUpSuccess`/`logInSuccess`/`changePasswordSuccess` take no payload), `redux/storage/legacyRootMigration.js` (pure split/verify helpers). Mobile Tier 1 = `react-native-mmkv` 4.3.2 AES-256 with a 32-char key in the secure store (`src/redux/storage/{bootstrap,mmkvStorage,wipe}.js`, reinstall detection); web Tier 1 = IndexedDB with `plain` and DEK-sealed `sealed` stores. `security/__fixtures__/vault.v1.json` is asserted in both repos' Jest runs and is the cross-platform envelope proof; never edit it in place. `src/redux/storage/persistTiming.js` is the dev-only R9a baseline hook (`serialize` in the persist config). Never branch on platform inside the submodule; every submodule change must be checked against both apps.
- **Secure storage runtime (Phase 2)**: The migrator never enrols biometrics: on Android react-native-sensitive-info shows the BiometricPrompt on every biometric-protected *write*, and on both platforms its 6.x resolver silently downgrades to a weaker policy (`devicePasscode`, even `none`) when biometrics cannot be used — never a boot-time prompt, never a DEK behind a weaker gate. Enrolment happens after the first password unlock (`unlockFlow.ensureBiometricEnrolled`) and only when `vault.isBiometricAvailable()` (adapter: `FingerprintScanner.isSensorAvailable()`) is true. RNSI 6 API: `service` (not `keychainService`), `authenticationPrompt` on reads and writes, `getItem` → `{value, metadata} | null` on both platforms, `deleteItem` → boolean, `setItem` → `{metadata}` with the policy actually applied, writes capped at 1 MiB, default `accessControl` is `secureEnclaveBiometry` so the adapter always sends one; `secureStore.set` deletes and throws `unavailable` when a protected write came back downgraded; the adapter maps the prompt for iOS (`{title, description}` — RNSI 6 puts `subtitle` on the fallback button). Errors are typed (`react-native-sensitive-info/errors` predicates); Android reports an invalidated Keystore key as a plain message (`/key (invalidated|unavailable)/`), which the adapter maps to `KEY_INVALIDATED`. There is no lockout code in 6.x: lockout reaches Login's default branch ("Biometric unlock is not available right now"); `LOCKED_OUT`/`BIOMETRIC_LOCKED_OUT` remain for the web adapter. No patch-package patch is needed any more. **Android 5.6.2 → 6 store**: `android/app/src/main/java/com/coinswallet/SensitiveInfoV6Migration.kt` (run from `MainApplication.onCreate`, JUnit test under `android/app/src/test`) re-shapes every unprotected 5.6.2 entry (`sensitive_info.xml`, key `service::key`) into the 6.x file `sensitive_info_<sha256(service)[0:8]>.xml` — same ciphertext/IV/Keystore alias, nothing decrypted, no prompt; protected entries are skipped and re-enrolled after the password unlock; marker `rnsi_v6_migration.xml` (boolean `done`, string `state`=`done` / `error`, and a per-entry `entry.<service>::<key>` = `migrated` | `exists` | `skipped`, all committed only after the re-shaped data; read from JS through `NativeKeygen.getLegacySecureValue`); a failed run never blocks `loadReactNative` — it records `error`, and `readLegacyRoot` refuses to treat a still-present 5.6.2 `persist:root2` entry as "no legacy blob" unless its own entry status is `migrated`/`exists` (no status → `migrate_native_store`, retried natively on the next launch; `skipped` → `migrate_parse`, both → StorageErrorScreen); the 5.6.2 file is retained (remove with the other legacy readers). iOS needs no migration (identical Keychain item shape). The iOS simulator does not enforce Keychain access-control flags, so biometric unlock must be QA'd on a physical device. Emulator: `adb emu finger touch <id>` drives the prompt. `src/redux/store.js` is `combineReducers` of one `persistReducer` per persisted slice (`persist:<slice>` in MMKV, `timeout: 0`, NO `throttle` — redux-persist serialises one top-level field per throttle tick and writes a slice only after every changed field, so `throttle: 1000` delayed the first wallets write by ~9 s and a quick quit lost a new wallet — plus a `persistFlush` listener that calls `persistor.flush()` on `IMMEDIATE_VAULT_WRITE_ACTIONS` and `resetWallet`, field-level transforms from `redux/storage/persistTransforms.js`), plus the `vaultSync` listener (`security/vaultSync.js`: immediate vault write for wallet-creating actions, debounced otherwise; `resetWallet` empties the vault, `logOutSuccess` destroys it). Boot: `App.js` awaits `bootstrapStorage()` (reinstall detection, MMKV key, open, `migrateLegacyRoot2` when `storage.schemaVersion` is 0 — `readLegacyRoot` reads the pre-RNSI-5 Android SharedPreferences blob directly instead of copying it through RNSI, whose 6.x writes are capped at 1 MiB; a migration failure keeps the promise rejected and shows `StorageErrorScreen`), then lazily requires `MainApp`, which wraps `Main` in `PersistGate`. `index.js` sets `setBootstrapContext('headless')` before requiring the store so the notifee task never runs the 600k KDF (partial migration, redone on the next foreground launch). Login goes through `src/security/unlockFlow.js` thunks (`unlockWithPassword` / `unlockWithBiometric` → `hydrateWalletSecrets` → refuse wallets without keys → `vaultUnlocked` → finalize migration 2→3 → enrol biometric copy if the setting is on); every former `=== password` check is `vault.verifyPassword`, ChangePassword is `vault.changePassword`, Registration is `vault.createVault`, Delete-all-data is `wipeAllLocalData`. Routing uses `getHasAccount`; WalletConnect starts only once `auth.isVaultUnlocked` (transient, blacklisted) is true; the WC transaction modal signs with `selectLivePrivateKey`. Web mirrors this with plain (`auth`, `settings`) and DEK-sealed IndexedDB slices: `unlockFlow.js` there unseals reads, `getStoredState` + `persist/REHYDRATE` per sealed slice, then enables writes. Legacy blobs are retained until the first unlock finalises them.
...
- **Secure storage — slimming (Phase 4)**: the shared wallets transform (`redux/storage/persistTransforms.js`) persists at most the 200 most recent `coins[*].transactions` per coin (by `date`; a refresh replaces the list wholesale), never persists the `nft` cache (refetched by `fetchNft`), strips list-shaped fields (`transactions`, `deriveAddresses`, `UTXOs`, `staking`, `nft`) from WalletConnect `walletData` entries, and `stripWalletSecrets` also strips the `selectedNft.coin` snapshot (a full live-coin copy with every derive key that `setSelectedNft` stores; cleared by `resetNfts` on launch and never re-hydrated — it was one source of the `secrets left in migrated wallets … under privateKey` self-check failure). `chain_existing_coin` entries are reduced to `CHAIN_EXISTING_PUBLIC_FIELDS` by the strip. `syncAllCoins` and `setWalletChainExistingCoin` build them through `toChainExistingEntry`; `extractChainExistingCoins` builds new entries directly from the six explicit fields and runs existing ones through `toChainExistingEntry`. `syncAllCoins` used to store the whole coin there (every Bitcoin derive key), the other source of that failure (KIMLWALLET-APP-8). Two more live-coin copies outside the wallets slice are stripped by the shared sanitizers in `legacyRootMigration.js`, which the runtime transforms now delegate to: `sellCrypto.requestDetails.selectedFromAsset` (`initiateSellCryptoTransfer` re-resolves the live wallet + coin before signing) and `batchTransaction.filteredData.filteredTransactions[*].coinInfo` (derived state, recomputed by `initializeFilters`). `splitLegacyRoot` additionally key-name-scans every non-wallet slice, deep-strips any hit and returns `residualSecrets`, which the mobile migrator reports as a Sentry warning (`step=sanitize_other_slices`), never a failure — the session UI only reads address/symbol/chain fields, and nothing rebuilds `walletData` from sessions, so it must stay persisted. `createMessagePersistTransform` (mobile only) keeps the newest 100 messages per XMTP conversation inside the encrypted store; there is no plaintext message cache by design. Limits live in `WALLET_PERSIST_LIMITS`. Measuring R9b: Settings › "Log persist timing" (dev / `SENTRY_DEV_TOOLS`) on mobile, `window.__dokPersistTiming()` in a dev web build; compare against the Phase 1 baseline on the same wallet set and put the numbers in the PR.
- **Secure storage — deliberately still present until migration coverage is confirmed**: `crypto-js` and the inlined `REDUX_WEB_KEY` on web (the only way to decrypt an un-migrated `persist:root`), the pre-RNSI-5 Android SharedPreferences fallback in `migrateLegacyRoot2.readLegacyRoot` / `wipe.removeLegacyRootBlob`, the legacy `persist:root2` reader in `migrateLegacyRoot2.js`, and the retained 5.6.2 `sensitive_info.xml` left by `SensitiveInfoV6Migration.kt`. Remove them only when Sentry shows `storage.migration` finalised for ~100 % of active installs; removing earlier makes every un-migrated user's upgrade fail with the storage error screen. `redux-persist-sensitive-storage` (mobile) and `redux-persist-transform-encrypt` (web) are already removed.
...
- **Error reporting (Sentry)**: `@sentry/react-native`, one project per variant (`DOK_WALLET_SENTRY_DSN` / `KIML_WALLET_SENTRY_DSN`, selected by `wlName` in `src/utils/wlData.js`). All observability goes through `src/services/logger` (`initSentry`, `logger.{debug,info,warn,error}`, `captureError(err, {tags, extra, level})`, `addBreadcrumb`, `setUserContext`); never import `@sentry/*` elsewhere. Captured: uncaught errors + unhandled rejections, native crashes, both ErrorBoundaries, every `console.*` as a Sentry Log (Bugfender parity), and structured logs at chokepoints (send, wallet create/import, WalletConnect, `dokapi.failed`, every `*/rejected` thunk via a store middleware, navigation/lifecycle/toast breadcrumbs). `user.id` is the persisted `masterClientId`; tag/attribute `variant`. `scrub.js` redacts mnemonics, hex/WIF/xprv keys, sensitive JSON keys and axios bodies in `beforeSend`/`beforeBreadcrumb`/`beforeSendLog`; call sites must still not pass addresses or amounts (only `tx_hash` is allow-listed). Disabled in debug builds unless `SENTRY_ENABLE_IN_DEV=true`; `SENTRY_DEV_TOOLS=true` shows a "Send Sentry test event" row in Settings. No source-map upload yet (JS frames are minified in the dashboard). Migration verify/parse failures (`step=verify` / `step=parse`) carry structure-only diagnostics in `extra`: `leakedPathPatterns` (normalized paths such as `allWallets[*].selectedNft.coin.privateKey x151`), `walletsDiff` / `vaultDiff` (first differing paths with value-free shapes), `walletFieldInventory`, `counts`, `slice`. Dynamic keys (addresses, uuids, derive paths) become `<key>`, strings appear only as `string(len)`; everything is a string VALUE under a neutral key because `scrubObject` drops any key that looks sensitive (`secretPathPatterns` would have been silently removed). Secret field names are also aliased in every diagnostic string (`privateKey`→`PK`, `extendedPrivateKey`→`XPRV`, `phrase`→`MNEM`, `secretCode*`→`SC_*`, other matches → `<sens>`; `SECRET_FIELD_ALIASES` / `SENTRY_SENSITIVE_VALUE_RE`) and problem messages avoid the word "secret": Sentry's server-side `@password:filter` replaces any string VALUE containing `privatekey`/`secret`/`password`/`auth` with `[Filtered]` (it wiped KIMLWALLET-APP-8's patterns); `_meta` on the raw event (`/api/0/organizations/<org>/issues/<id>/events/latest/`) names the rule and the original length. `verify` failures also carry `leakContext` (aliased parent/grandparent shapes per pattern) and `sliceRootInventory`.
...
- After switching branches that add or remove native deps, let `yarn install` finish before starting the Android build. The RN settings plugin re-runs `react-native config` only when the SHA of `yarn.lock`/`package.json`/`react-native.config.js` changes, and stamps the new SHAs even if `node_modules` is still the old branch's; a build that starts mid-install pins a config missing the new module (symptom: Nitro "HybridObject X has not yet been registered", e.g. `MMKVFactory`, and `StorageErrorScreen` at launch). Fix: `rm -rf android/build/generated/autolinking` and rebuild.
