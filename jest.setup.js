...
// react-native-sensitive-info 5.6.2 (Nitro) has no Jest mock of its own. This
// in-memory one keeps the semantics src/security/secureStore.js relies on:
// items are namespaced by keychainService; Android throws E_NOT_FOUND for a
// missing key where iOS returns null; biometric-protected items count an OS
// prompt on every protected read/write; `__mock.invalidate(key)` simulates the
// Keystore destroying a key after an enrollment change (E_KEY_INVALIDATED).
jest.mock('react-native-sensitive-info', () => {
  const {Platform} = require('react-native');
  const items = new Map();
  const state = {prompts: 0, invalidated: new Set(), cancelNext: false};
  ...
  const api = {
    ...
    setItem: jest.fn(async (key, value, options) => {
      if (isProtected(options?.accessControl)) {
        state.prompts += 1;
      }
      items.set(id(key, options), {
        value,
    ...
    getItem: jest.fn(async (key, options) => {
      ...
      if (!item) {
        if (Platform.OS === 'android') {
          throw err('E_NOT_FOUND', `Failed to retrieve "${key}"`);
        }
        return null;
      }
      if (isProtected(item.accessControl)) {
        ...
        if (state.invalidated.has(id(key, options))) {
          throw err('E_KEY_INVALIDATED');
        }
        if (state.cancelNext) {
          state.cancelNext = false;
          throw err('E_AUTH_CANCELED');
        }
      }
      return item.value;
    }),
    hasItem: jest.fn(async (key, options) => items.has(id(key, options))),
    deleteItem: jest.fn(async (key, options) => {
